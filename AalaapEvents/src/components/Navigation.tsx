  ...
  return (
    <>
      <motion.header
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'glass-nav-scrolled' : 'glass-nav'
          }`}
      >
          ...
          <div className="flex items-center justify-between h-24 sm:h-28 lg:h-32">
            ...
            <nav className="hidden lg:flex items-center gap-1">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`relative px-4 py-2 text-[0.8rem] font-medium tracking-wide uppercase transition-all duration-300 ${location.pathname === link.path
                      ? 'text-gold'
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`lg:hidden w-9 h-9 flex items-center justify-center rounded-full border transition-all duration-300 cursor-pointer ${isMobileOpen
            ...
            >
                ...
                <div className="space-y-0.5">
                  {navLinks.map((link, i) => (
                    ...
                    >
                      <Link
                        to={link.path}
                        className={`flex items-center justify-between py-3 px-3 rounded-lg text-[0.95rem] font-display font-semibold transition-all duration-200 ${location.pathname === link.path
              ...
              >
                ...
                >
                  ...
                  <div>
                    <p className="text-xs font-semibold text-text-primary group-hover:text-gold transition-colors">@aalaap_entertainers</p>
                    <p className="text-[0.65rem] text-gold font-medium">Event Management</p>
                  </div>
                </a>
                <div className="flex items-center gap-1.5 text-text-muted text-[0.65rem]">
                  <MapPin className="w-3 h-3 text-gold flex-shrink-0" />
                  Madanayakanahalli, Bengaluru
                </div>
              </motion.div>
            </motion.div>
          </>
